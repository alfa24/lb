...
<li class="<?php echo ($imgsrc[0] == "") ? 'no-img-post' : ''; ?>">
  ...
  <div class="post-text-bx">
    ...
    <?php } ?>
    <div class="post-meta"> <span class="post-date"><i class="fa fa-calendar-o"></i><a href="<?php echo esc_url(get_permalink($recent["ID"])); ?>"><?php echo date('d M Y',strtotime(esc_html($recent["post_date"]))); ?></a></span> <span class="post-date"><i class="fa fa-comments"></i><?php echo '<a href="'.esc_url(get_comments_link($recent["ID"])).'">'.esc_html($recent["comment_count"]).'</a>'; ?></span> </div>
  </div>
</li>
<?php
      } 
...
<p>
  <label for="<?php echo esc_html($this->get_field_id('title')); ?>"><?php esc_html_e('Title', 'service-finder'); ?>:
  <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo  esc_attr($title); ?>" />
  </label>
</p>
<?php
            ...
            <ul>
              <?php 
				...
				if(!empty($providers)){
					...
					$bookingurl = service_finder_get_author_url($providerid);
					$src = '';
