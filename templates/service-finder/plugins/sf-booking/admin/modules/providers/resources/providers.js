...
jQuery('body').on('click', 'input[name="makefeatured"]', function(){
	proid = jQuery(this).val();
			...
			.on('shown.bs.modal',function () {
				...
				bootbox.confirm("Do you want to make this provider Unfeatured?", function(result) {
				  if(result){
					  var data = {
...
/*Un-Block User*/ 
  jQuery('body').on('click', '.unblockaccount', function(){
			uid = jQuery(this).attr('data-id');
				...
				bootbox.confirm("Do you want to make this provider Unblocked?", function(result) {
				  if(result){
					  var data = {
								  "action": "unblock_user",
								  "uid": uid
