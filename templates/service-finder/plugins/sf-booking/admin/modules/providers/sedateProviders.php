...
class SERVICE_FINDER_sedateProviders extends SERVICE_FINDER_sedateManager{
	...
	public function service_finder_get_providers(){
		...
		$providers = $wpdb->get_results('SELECT provider.id, provider.wp_user_id, provider.admin_moderation, provider.account_blocked, provider.full_name, provider.mobile, provider.phone, provider.category_id, provider.email, provider.city, featured.amount, featured.days, featured.status,featured.feature_status FROM '.$service_finder_Tables->feature.' as featured RIGHT JOIN '.$service_finder_Tables->providers.' as provider on featured.provider_id = provider.wp_user_id');
...
'.esc_html__('Featured (By Admin)', 'service-finder');
			...
			$status = '
<input type="checkbox" name="makefeatured" value="'.esc_attr($result->wp_user_id).'" id="makefeatured-'.esc_attr($result->wp_user_id).'">
';
			}
			$nestedData[] = $status;
			...
			if($result->admin_moderation == 'approved'){
				...
				$nestedData[] = '<a href="javascript:;" data-id="'.esc_attr($result->wp_user_id).'" class="unblockaccount btn btn-primary btn-xs">'.esc_html__('UnBlock/Approve After Wired Transfer', 'service-finder').'</a> <a href="'.esc_url($manageprofilelink).'" target="_blank" class="btn btn-primary btn-xs">'.esc_html__('Manage Profile', 'service-finder').'</a>';
				}else{
	...
	public function service_finder_get_providers_identity(){
		...
		foreach($query as $result){
			...
			if(!empty($attachmentIDs)){
				...
				$identityfile = '<a href="'.SERVICE_FINDER_BOOKING_LIB_URL.'/downloads.php?file='.wp_get_attachment_url( $attachmentID->attachmentid ).'"><i class="fa fa-download"></i> '.esc_html__('View/Download Identity').'</a>';
				}
			}else{
	...
	$wpdb->query($wpdb->prepare('UPDATE '.$service_finder_Tables->providers.' SET `account_blocked` = "no" WHERE `wp_user_id` = %d',$_POST['uid']));
