...
?>
  <div class="clear" id="comment-list">
	<div class="comments-area" id="comments">
          ...
          <h2 class="comments-title"> <?php printf( esc_html__('%d Reviews', 'service-finder' ), $totalreview ); ?> </h2>
		<div id='contentInner'>
        <!-- comment list END -->
		...
		if(!empty($reviews)){
			...
			if(!empty($avatar_id) && $avatar_id > 0){
					...
					$src = 'http://2.gravatar.com/avatar/2d8b3378fb00ca047026e456903cae16?s=56&d=mm&r=g';
			}		
			?>
...
<div class="show-rating-bx"><?php echo service_finder_displayRating($review->rating); ?></div>
<p><?php echo $review->comment; ?></p>
		...
		<div align="center">
		<ul class='pagination text-center' id="bookingpagination">
		<?php if(!empty($total_pages)):for($i=1; $i<=$total_pages; $i++):  
					if($i == 1):?>
					<li class='active' id="<?php echo $i;?>" data-link="<?php echo $authorurl ?>"><a href='<?php echo $authorurl.'/?comment_page='.$i ?>'><?php echo $i;?></a></li> 
					<?php else:?>
					<li id="<?php echo $i;?>" data-link="<?php echo $authorurl ?>"><a href='<?php echo $authorurl.'/?comment_page='.$i ?>'><?php echo $i;?></a></li>
				<?php endif;?>			
		<?php endfor;endif;?>  
		</ul>
		</div>
