...
$res = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->notifications.' WHERE `provider_id` = %d AND `read` = "no" ORDER BY id DESC',$current_user->ID));
$count = count($res);
$html .= '<a href="javascript:;" data-delay="1000" data-hover="dropdown" data-toggle="dropdown" data-usertype="Provider" data-userid="'.$current_user->ID.'" class="dropdown-toggle btn btn-border btn-sm sf-notifications" aria-expanded="false"><i class="fa fa-bell"></i> <span>'.esc_attr($count).'</span></a>';
if($count == 0){
$res = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->notifications.' WHERE `provider_id` = %d ORDER BY id DESC LIMIT 5',$current_user->ID));
}
}elseif(service_finder_getUserRole($current_user->ID) == 'Customer'){
$res = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->notifications.' WHERE `customer_id` = %d AND `read` = "no" ORDER BY id DESC',$current_user->ID));
$count = count($res);
$html .= '<a href="javascript:;" data-delay="1000" data-hover="dropdown" data-toggle="dropdown" data-usertype="Customer" data-userid="'.$current_user->ID.'" class="dropdown-toggle btn btn-border btn-sm sf-notifications" aria-expanded="false"><i class="fa fa-bell"></i> <span>'.esc_attr($count).'</span></a>';
if($count == 0){
$res = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->notifications.' WHERE `customer_id` = %d ORDER BY id DESC LIMIT 5',$current_user->ID));
}
}
if(!empty($res)){
	$html .= '<ul class="dropdown-menu arrow-up">';
	foreach($res as $row){
	if($row->read == 'yes'){
		$class = 'sf-read';
	}else{
