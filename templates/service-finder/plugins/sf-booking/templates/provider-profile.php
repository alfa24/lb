...
?>
  ...
  $classes = (service_finder_themestyle() == 'style-2') ? 'sf-search-result pos-v-center' : '';
  ?>
  <div class="sf-find-bar-inr <?php echo $classes; ?>">
      <div class="container">
  ...
  <div class="container">
    <div class="row section-content provider-content">
      ...
      <div class="col-md-8">
        ...
        <div class="padding-20  margin-b-30  bg-white sf-rouned-box">
          <div class="provider-details clearfix">
            <div class="provider-logo">
              <?php
				...
				if(!empty($providerInfo->avatar_id) && $providerInfo->avatar_id > 0){
					$src  = wp_get_attachment_image_src( $providerInfo->avatar_id, 'service_finder-provider-medium' );
					$src  = $src[0];
				}
				
				$src = (!empty($src)) ? $src : service_finder_get_default_avatar();
				if($src != ''){
			...
			?>
              ...
              <p><?php echo nl2br(stripcslashes($providerInfo->bio)); ?></p>
            </div>
          </div>
          <div class="provider-social clearfix">
            <ul class="social-bx list-inline">
              ...
              <?php } ?>
              <?php if($providerInfo->google_plus != ""){ ?>
              <li><a href="<?php echo esc_url($providerInfo->google_plus); ?>" target="_blank" rel="nofollow" class="fa fa-google-plus"></a></li>
              <?php } ?>
        ...
        <div class="padding-20  margin-b-30  bg-white sf-rouned-box">
          <div class="shared-bx clearfix">
            ...
            <ul class="share-social-bx">
              ...
              <?php } ?>
              <?php if($service_finder_options['google-plus']){ ?>
              <li class="gp"><a onclick="javascript:window.open('https://plus.google.com/share?url=<?php echo service_finder_get_author_url($author) ?>', '_blank', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;" href="javascript:;"> <i class="fa fa-google-plus"></i>
                <?php esc_html_e('Share', 'service-finder'); ?>
                </a></li>
              <?php } ?>
        ...
        <?php 
		if(service_finder_showBusinessHours($author)){
		?>
        ...
        <div class="padding-20  margin-b-30  bg-white bh-section sf-rouned-box"> <?php echo service_finder_showBusinessHours($author); ?> </div>
        ...
        <div class="padding-20  margin-b-30  bg-white sf-rouned-box">
          ...
          <ul class="provider-info clearfix no-margin equal-col-outer">
             ...
             <?php } ?>
             <?php if(service_finder_get_contact_info($providerInfo->phone,$providerInfo->mobile) != "" && $service_finder_options['show-contact-number']){ ?>
             <li class="equal-col"><i class="fa fa-phone"></i>
             	<strong><?php esc_html_e( 'Telephone', 'service-finder' ) ?>:</strong>
				 <span><?php echo service_finder_get_contact_info($providerInfo->phone,$providerInfo->mobile); ?></span>
             </li>
             <?php } ?>
        ...
        <!-- End Address Section-->
        <?php if($service_finder_options['show-contact-map']){ ?> 
            ...
            <div class="provider-location">
              <div id='gmap_canvas map-canvas-in'>
                <?php
						...
						$marker = '[\"'.$providerInfo->full_name.'\",\"'.$providerInfo->lat.'\",\"'.$providerInfo->long.'\",\"'.$src.'\",\"'.$icon.'\",\"'.$userLink.'\",\"'.$providerInfo->wp_user_id.'\",\"'.service_finder_getCategoryName(get_user_meta($providerInfo->wp_user_id,'primary_category',true)).'\",\"'.$markeraddress.'\",\"'.stripcslashes($companyname).'\"]';
	  ...
	  if(!empty($results)){
		...
		if($i == 3){
		echo '<div class="showalllocation default-hidden">';
		}
		?>
            <li class="equal-col">
      ...
      if(count($results) > 2){
	  ?>
      <div class="sf-location-btn"><a class="btn btn-primary showmorelocation" href="javascript:;"><?php esc_html_e('Show More Location', 'service-finder'); ?></a></div>
      <?php } ?>
      </div>
            </div>
          </div>
		...
		if(!empty($embeded_codes)){
			?>
			<h4>
			  <?php esc_html_e( 'Our Video', 'service-finder' ) ?>
            </h4>
			<?php
			if($totalvideos > 1){
			echo '<div class="padding-20  margin-b-30  bg-white sf-rouned-box">';
        ...
        <!-- Our Services-->
        <h4>
          <?php echo (!empty($service_finder_options['label-our-services'])) ? esc_html($service_finder_options['label-our-services']) : esc_html__('Our services', 'service-finder'); ?>
        </h4>
        <div class="padding-20  margin-b-30  bg-white sf-rouned-box">
          <div class="tabbable">
            <ul class="nav nav-tabs">
              <li class="active"><a data-toggle="tab" href="#box1">
                <?php echo (!empty($service_finder_options['label-services'])) ? esc_html($service_finder_options['label-services']) : esc_html__('Services', 'service-finder'); ?>
                </a></li>
              <li><a data-toggle="tab" href="#box2">
            ...
            <div class="tab-content">
              <div id="box1" class="tab-pane fade in active">
              	<?php 
								$services = service_finder_getServices($providerInfo->wp_user_id);
								$i=0;
								$serviceflag = 0;
								if(!empty($services)){
								$serviceflag = 1;
								echo '<div class="provider-services"><ul>';
								...
								if($service->cost > 0){
									...
									if($service->hours > 0){
									$cost = service_finder_money_format($service->cost).'<br/>';
									$cost .= '<span class="sf-fix-hours"><i class="fa fa-clock-o"></i> '.$service->hours.esc_html__(' Hrs', 'service-finder').'</span>';
									}else{
									$cost = service_finder_money_format($service->cost).esc_html__('/Hr', 'service-finder');
									}
									...
									$cost = service_finder_money_format($service->cost).'<br/>';
									$cost .= '<span class="sf-fix-hours"><i class="fa fa-user"></i> '.$service->persons.esc_html__(' Items', 'service-finder').'</span>';
									}else{
									$cost = service_finder_money_format($service->cost).esc_html__('/Person', 'service-finder');
									}
									
									}else{
									$cost = service_finder_money_format($service->cost);
									}
									
									
									
								}else{
									$cost = '<a href="#form-quot-bx"><i class="fa fa-edit"></i></a>';
								}
								
								
									echo '<li>';	
									echo '<i class="fa fa-check"></i>';
									echo '<h6 class="p-s-title">'.stripcslashes($service->service_name).' <span class="cost">'.$cost.'</span></h6>';
									echo '<p class="p-s-text">'.nl2br(stripcslashes($service->description)).'</p>';
									echo '</li>';
								}
								echo '</ul></div>';	
								}
								
								$groups = service_finder_getServiceGroups($providerInfo->wp_user_id);
								if(!empty($groups)){
									foreach($groups as $group){
										
										$services = service_finder_getServices($providerInfo->wp_user_id,'',$group->id);	
										$i=0;
										if(!empty($services)){
										$serviceflag = 1;
										echo '<div class="provider-services provider-service-group">';
										echo '<h4 class="sf-title">'.$group->group_name.'</h4>';
										echo '<ul>';
											foreach($services as $service){
								...
								echo '<table class="table borderless margin-0 sf-documents-table">';
                                	...
                                	<div class="panel panel-default">
                                        <div class="panel-heading">
                                            <strong class="price-bx"><a href="'.SERVICE_FINDER_BOOKING_LIB_URL.'/downloads.php?file=%s"><i class="fa fa-download"></i></a></strong>
...
<label for="invoicepaypal"><img src="'.SERVICE_FINDER_BOOKING_IMAGE_URL.'/payment/paypal.jpg" alt="'.esc_html__('paypal','service-finder').'"></label>
</div>
';
$payflag = 1;
}
...
<label for="invoicepayumoney"><img src="'.SERVICE_FINDER_BOOKING_IMAGE_URL.'/payment/payumoney.jpg" alt="'.esc_html__('PayU Money','service-finder').'"></label>
</div>
';
$payflag = 1;
}
...
<label for="invoicepayulatam"><img src="'.SERVICE_FINDER_BOOKING_IMAGE_URL.'/payment/payulatam.jpg" alt="'.esc_html__('PayU Latam','service-finder').'"></label>
</div>
';
$payflag = 1;
}
...
$payflag = 1;
		...
		$payform = '<form class="myform pay-now" method="post">
								<div class="col-md-12">
									...
									<div id="invoicecardinfo" class="default-hidden">
                                      <div class="col-md-8">
    ...
    <div class="col-md-12">
	  <div class="form-group">
	   ...
	   <select id="payulatam_invoice_cardtype" name="payulatam_invoice_cardtype" class="form-control"  title="'.esc_html__('Select Card', 'service-finder').'">';
    ...
    <div class="col-md-6">
      <div class="form-group has-select">
        <label>'.esc_html__('Select Month', 'service-finder').'</label>
        <select id="payulatam_card_month" name="payulatam_card_month" class="form-control" title="'.esc_html__('Select Month', 'service-finder').'">
    ...
    <div class="col-md-6">
      <div class="form-group has-select">
        <label>'.esc_html__('Select Year', 'service-finder').'</label>
        <select id="payulatam_card_year" name="payulatam_card_year" class="form-control"  title="'.esc_html__('Select Year', 'service-finder').'">
    ...
    <div class="col-md-6">
	  ...
	  ?>
		...
		<?php
			...
			?>
            <div class="padding-20 margin-b-30 bg-white sf-rouned-box">
			<strong class="sf-job-title-bx">
			<?php esc_html_e( 'Job: ', 'service-finder' ) ?>
			<?php echo get_the_title($jobid);  ?>
			</strong>
            </div>
			<?php } ?>
	  ...
	  ?>
        ...
        <h4>
      	...
      	<div class="padding-20  margin-b-30  bg-white review-section sf-rouned-box">
      ...
      <div class="col-md-4">
        ...
        <h4>
          <?php echo (!empty($service_finder_options['label-related-provider'])) ? esc_html($service_finder_options['label-related-provider']) : esc_html__('Related Providers', 'service-finder'); ?>
        </h4>
        <div class="padding-20  margin-b-30  bg-white sf-rouned-box">
...
<div class="page-content">
  ...
  $classes = (service_finder_themestyle() == 'style-2') ? 'sf-search-result pos-v-center' : '';
  ?>
  <div class="sf-find-bar-inr wow fadeInDown <?php echo $classes; ?>">
      <div class="container">
      <?php if(service_finder_themestyle() == 'style-2'){ ?>
      <ul class="sf-search-title clearfix">
            <li><?php echo esc_html__('Search Provider', 'service-finder'); ?></li>
    ...
    <div class="container">
      <div class="row">
          ...
          <div class="sf-provider-des">
            <div class="sf-thum-bx overlay-black-light">
              <?php
						...
						echo $imgtag;
						?>
              <div class="rating-bx"> <?php echo service_finder_displayRating(service_finder_getAverageRating($author)); ?> </div>
            </div>
            <span class="sf-provider-name">
            <?php the_author_meta( 'first_name', $author ) ?>
            <?php the_author_meta( 'last_name', $author ); ?>
        ...
        <div class="sf-about-box">
          <h2 class="sf-title"><?php echo service_finder_getCompanyName($providerInfo->wp_user_id) ?></h2>
          <span class="tagline"><?php echo esc_html($providerInfo->tagline); ?></span>
          ...
          <div class="shared-bx">
            <ul class="sharebtn-bx">
              ...
              <?php } ?>
              <?php if($service_finder_options['request-quote'] && service_finder_request_quote_for_loggedin_user()){ ?>
              <li><a href="javascript:void(0);" data-providerid="<?php echo esc_attr($author);?>" data-toggle="modal" data-target="#quotes-Modal">
        <i class="fa fa-hand-o-right"></i> <?php echo esc_html($requestquote);  ?>
        </a> </li>
        <?php } ?>
            </ul>
            <?php if($service_finder_options['enable-social-shares']){ ?>
  ...
  <!-- Gallery row -->
  <?php 
  $gallerybgurl = (!empty($service_finder_options['photo-gallery-bg-image']['url'])) ? $service_finder_options['photo-gallery-bg-image']['url'] : '';
  $bgattachment = (isset($service_finder_options['photo-gallery-background-attachment'])) ? esc_html($service_finder_options['photo-gallery-background-attachment']) : '';
  $bgcolor = (!empty($service_finder_options['photo-gallery-bg-color'])) ? $service_finder_options['photo-gallery-bg-color'] : '';
  $bgopacity = (!empty($service_finder_options['photo-gallery-bg-opacity'])) ? $service_finder_options['photo-gallery-bg-opacity'] : '';

   ?>
  <?php $images = service_finder_getProviderAttachments($providerInfo->wp_user_id,'gallery'); ?>
  <?php if(!empty($images)){ ?>
  <section class="section-full overlay-black-middle  text-center gallery-row " style=" <?php echo 'background:url('.$gallerybgurl.') center '.$bgattachment.' no-repeat;'; ?> ">
    <div class="container">
      ...
      <div class="section-content">
        <div class="gallery-thums2 gallery popup-gallery">
          
          <?php
							if(!empty($images)){
							foreach($images as $image){
								$src  = wp_get_attachment_image_src( $image->attachmentid, 'service_finder-gallery-v2' );
								...
								$html = sprintf('<div class="item">
									<div class="sf-thum-bx img-effect1">
										...
										<div class="overlay-bx">
											<div class="overlay-icon">
												<a href="%s" data-gal="prettyPhoto[gallery1]">
    ...
    <div class="sf-overlay-main" style="opacity:<?php echo $bgopacity ?>; background-color:<?php echo $bgcolor ?>;">
    </div>
  </section>
  <?php } ?>
  <!-- Gallery row END -->
  <?php if(service_finder_showBusinessHours($author)){ ?>
  <!-- business hours row -->
  <section class="section-full text-center bg-white bh-row">
    <div class="container">
      ...
      <div class="section-content"> <?php echo service_finder_showBusinessHours($author); ?></div>
    </div>
  </section>
  <!-- business hours row END -->
  <?php } ?>
  <?php if($service_finder_options['show-address-info'] && service_finder_check_address_info_access()){ ?>
  <!-- address-info -->
  <section class="section-full text-center bg-gray address-info">
    <div class="container">
      <div class="section-head">
        <h2 class="text-uppercase">
          <?php esc_html_e('Address Info', 'service-finder'); ?>
        </h2>
      </div>
      <div class="section-content equal-col-outer">
        <div class="row">
        <?php if(service_finder_getAddress($author) != "" && $service_finder_options['show-postal-address']){ ?>
          <div class="col-md-4 col-sm-6 equal-col">
            <div class="sf-element-bx padding-lr-30">
              ...
              <p><?php echo service_finder_getAddress($author); ?></p>
            </div>
          </div>
        <?php } ?>  
        <?php if(service_finder_get_contact_info($providerInfo->phone,$providerInfo->mobile) != "" && $service_finder_options['show-contact-number']){ ?>
          <div class="col-md-4 col-sm-6 equal-col">
            <div class="sf-element-bx padding-lr-30">
              ...
              <h6>
                <?php esc_html_e('Telephone', 'service-finder'); ?>
              </h6>
              <p><?php 
			  echo service_finder_get_contact_info($providerInfo->phone,$providerInfo->mobile); ?></p>
            </div>
          </div>
        <?php } ?>    
        <?php if($providerInfo->fax != "" && $service_finder_options['show-fax']){ ?>    
          <div class="col-md-4 col-sm-6 equal-col">
            <div class="sf-element-bx padding-lr-30">
              <div class="icon-bx-md rounded-bx"> <i class="fa fa-fax"></i> </div>
              <h6>
                <?php esc_html_e('FAX', 'service-finder'); ?>
              </h6>
              <p><?php echo esc_html($providerInfo->fax); ?></p>
            </div>
          </div>
        <?php } ?>  
        <?php if($providerInfo->lat != "" && $providerInfo->long != "" && $service_finder_options['show-gps']){ ?> 
          <div class="col-md-4 col-sm-6 equal-col">
            <div class="sf-element-bx padding-lr-30">
              <div class="icon-bx-md rounded-bx"> <i class="fa fa-street-view"></i> </div>
              <h6>
                <?php esc_html_e('GPS', 'service-finder'); ?>
              </h6>
              <p><?php echo esc_html($providerInfo->lat).', '.esc_html($providerInfo->long); ?></p>
            </div>
          </div>
        <?php } ?>    
        <?php if($service_finder_options['show-email-address']){ ?> 
          <div class="col-md-4 col-sm-6 equal-col">
            <div class="sf-element-bx padding-lr-30">
              ...
              <p>
                <a href="mailto:<?php the_author_meta( 'user_email', $author ); ?>"><?php the_author_meta( 'user_email', $author ); ?></a>
              </p>
            </div>
          </div>
        <?php } ?>  
          ...
          <div class="col-md-4 col-sm-6 equal-col">
            <div class="sf-element-bx padding-lr-30">
              <div class="icon-bx-md rounded-bx"> <i class="fa fa-globe"></i> </div>
              <h6>
                <?php esc_html_e('WEB', 'service-finder'); ?>
              </h6>
  ...
  <section class="map-location">
    <div class='gmap-outer pro-map-outer-in'>
      <?php
						...
						wp_add_inline_script( 'service_finder-js-gmapfunctions', 'var googlecode_regular_vars = {"general_latitude":"'.esc_js($providerInfo->lat).'", "general_longitude":"'.esc_js($providerInfo->long).'","path":"'.esc_js($imagepath).'","markers":"['.$marker.']","idx_status":"0","page_custom_zoom":"'.esc_js($zoom_level).'","generated_pins":"0","scrollwheel":false};
						jQuery(document).ready(function($) {
									initializeSearchMap();
									});', 'before' );
		
		
      echo do_shortcode('[service_finder_map general_latitude="'.$providerInfo->lat.'" general_longitude="'.$providerInfo->long.'" height="500px" width="2000px;"]'); ?> </div>
      <div class="sf-location-gallery">
      <ul class="sf-location-listing equal-col-outer">
      <?php
      $results = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->branches.' WHERE wp_user_id = %d',$providerInfo->wp_user_id));
	  ...
	  if(!empty($results)){
		...
		?>
            <li class="equal-col sf-active-location">
               <a href="javascript:;" class="load-branch-address" data-branchid="<?php echo esc_attr($res->id); ?>" data-userid="<?php echo esc_attr($providerInfo->wp_user_id); ?>">
			   <?php
               echo service_finder_getBranches($res->id);
               ?>
  ...
  <?php if(!empty($providerInfo->embeded_code)){ ?>
  <!-- video-row -->
  <section class="section-full text-center bg-gray video-row">
    <div class="container">
      <div class="section-head">
        <h2 class="text-uppercase">
          <?php esc_html_e('Our Video', 'service-finder'); ?>
      ...
      <div class="section-content">
		...
		if(!empty($embeded_codes)){
			if($totalvideos > 1){
			echo '<div id="slider" class="owl-video-v2 sf-video-gallery owl-btn-center-lr">';
			$oembed = new ClassOEmbed();
			foreach($embeded_codes as $embeded_code){
			?>
				<div class="item">
  ...
  <?php } ?>
  <!-- our services row -->
  <section class="section-full bg-white services-row pro-bg-in">
    <div class="container">
      <div class="tabbable">
        <ul class="nav nav-tabs">
          <li class="active"><a data-toggle="tab" href="#box1">
        ...
        <div class="tab-content">
          <div id="box1" class="tab-pane fade in active">
			...
			<?php 
								...
								if(!empty($services)){
								$serviceflag = 1;
								echo '<div class="row"><ul class="sf-services-list">';
									foreach($services as $service){
									...
									if($service->cost > 0){
										...
										$cost = service_finder_money_format($service->cost).esc_html__('/Item', 'service-finder');
										}
										...
										$cost = '<a href="javascript:void(0);" data-providerid="'.esc_attr($author).'" data-toggle="modal" data-target="#quotes-Modal"><i class="fa fa-edit"></i></a>';
									}
									
										$html = sprintf('<li class="col-md-6 col-sm-6">
                                	<div class="panel panel-default">
                                        <div class="panel-heading" data-toggle="collapse" data-target="#sf-servive%d" aria-expanded="">
                                            <strong class="price-bx">%s</strong>
                                            <span class="service-title">%s</span>
                                        </div>
                                        <div class="panel-body collapse" id="sf-servive%d">
                                            <p>%s</p>
                                        </div>
                                    </div>
                                </li>',
													$i,
													$cost,
													stripcslashes($service->service_name),
													$i,
													nl2br(stripcslashes($service->description))
												);
													echo $html;	
								      $i++;
									}
								...
								if(!empty($groups)){
									foreach($groups as $group){
										...
										if(!empty($services)){
										$serviceflag = 1;
										echo '<div class="sf-service-categories clearfix clear">';
										echo '<h4 class="sf-title">'.$group->group_name.'</h4>';
										echo '<div class="row"><ul class="sf-services-list">';
											foreach($services as $service){
											...
											if($service->cost > 0){
												...
												$html = sprintf('<li class="col-md-6 col-sm-6">
											<div class="panel panel-default">
												<div class="panel-heading" data-toggle="collapse" data-target="#sf-servive-%d-%d" aria-expanded="">
													<strong class="price-bx">%s</strong>
													<span class="service-title">%s</span>
												</div>
												<div class="panel-body collapse" id="sf-servive-%d-%d">
													<p>%s</p>
												</div>
											</div>
										</li>',
...
$payflag = 1;
		...
		$payform = '<form class="myform pay-now" method="post">
								<div class="col-md-12">
									...
									<div id="invoicecardinfo" class="default-hidden">
                                      <div class="col-md-8">
                                    	<div class="form-group">
                                        	<label>'.esc_html__('Stripe', 'service-finder').'Card Number</label>
                                            <div class="input-group">
                                                <i class="input-group-addon fa fa-credit-card"></i>
                                                <input type="text" id="card_number" name="card_number" class="form-control">
                                            </div>
    ...
    <div class="col-md-6">
      <div class="form-group has-select">
		...
		$cuszipcode = (!empty($row->cuszipcode)) ? $row->cuszipcode : '';
		$html = '<section class="section-full bg-white invoice-row" id="invoiceview">
    <div class="container"><div class="invoice-view">
  ...
  <!-- book now form -->
  <section class="section-full bg-gray book-now-row book-now-scroll" id="book-now-section">
    <div class="container">
      <div class="section-head text-center">
        ...
        <span class="mincost">
        <?php esc_html_e( 'Job: ', 'service-finder' ) ?>
        <strong><?php echo get_the_title($jobid);  ?></strong>
        </span>
        <?php } ?>
        <?php if($settings['booking_option'] == 'paid' && $paid_booking){ ?> 
        <span class="mincost">
      ...
      <div class="section-content">
        <?php require SERVICE_FINDER_BOOKING_FRONTEND_MODULE_DIR . '/book-now/templates/book-now-v2.php'; ?>
      </div>
    </div>
  </section>
  <!-- book now form END -->
  ...
  <?php } ?>
  <?php if($service_finder_options['review-system']){ ?>
  <section class="section-full book-now-row book-now-comments bg-white review-section">
    <div class="container">
    	<div class="section-head">
        <h2 class="text-uppercase">
          <?php echo (!empty($service_finder_options['label-review'])) ? esc_html($service_finder_options['label-review']) : esc_html__('Review', 'service-finder'); ?>
  ...
  <?php if($service_finder_options['request-quote'] && service_finder_request_quote_for_loggedin_user()){ ?>
  <?php 
  $quotebgurl = (!empty($service_finder_options['request-quote-bg-image']['url'])) ? $service_finder_options['request-quote-bg-image']['url'] : '';
  $bgattachment = (isset($service_finder_options['request-quote-background-attachment'])) ? esc_html($service_finder_options['request-quote-background-attachment']) : '';
  $bgcolor = (!empty($service_finder_options['request-quote-bg-color'])) ? $service_finder_options['request-quote-bg-color'] : '';
  $bgopacity = (!empty($service_finder_options['request-quote-bg-opacity'])) ? $service_finder_options['request-quote-bg-opacity'] : '';
   ?>
  <section class="section-full sf-overlay-wrapper text-center quotes-row"  style=" <?php echo 'background:url('.$quotebgurl.') center '.$bgattachment.' no-repeat;'; ?> ">
    <div class="container">
      <div class="padding-lr-50 w-t-element">
        <p><?php echo (!empty($service_finder_options['quote-description'])) ? $service_finder_options['quote-description'] : ''; ?></p>
        <a href="javascript:void(0);" data-providerid="<?php echo esc_attr($author);?>" data-toggle="modal" data-target="#quotes-Modal" class="btn btn-primary btn-lg">
        <?php echo esc_html($requestquote);  ?>
        </a> </div>
    </div>
    <div class="sf-overlay-main" style="opacity:<?php echo $bgopacity ?>; background-color:<?php echo $bgcolor ?>;">
