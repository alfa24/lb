...
// When the browser is ready...
  jQuery(function() {
	...
	//Customer Captcha
	if (jQuery('#recaptcha_customersignuppage').length){
	...
	//Provider Captcha
	if (jQuery('#recaptcha_providersignuppage').length){
	...
	jQuery('.sf-register-page a[data-toggle="tab"]').on('shown.bs.tab', function(e){
		...
		if(target == '#customertab'){
			if (jQuery('#recaptcha_customersignup').length){
			reload_recaptcha('recaptcha_providersignuppage');
			}
		}else if(target == '#providertab'){
			if (jQuery('#recaptcha_providersignup').length){
	...
	if(selectedpackage != ""){
		...
		jQuery('.nav-tabs a[href="#customertab"]').tab('show');
	}
