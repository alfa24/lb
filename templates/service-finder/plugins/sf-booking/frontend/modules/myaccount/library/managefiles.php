...
if ( ! class_exists( 'SERVICE_FINDER_FileSpace' ) )
{
	class SERVICE_FINDER_FileSpace extends ClassFeild
	{
		/**
		...
		static function admin_enqueue_scripts()
		{
			wp_localize_script( 'rwmb-file', 'rwmbFile', array(
				'maxFileUploadsSingle' => esc_html__( 'You may only upload maximum %d file', 'service-finder' ),
				'maxFileUploadsPlural' => esc_html__( 'You may only upload maximum %d files', 'service-finder' ),
			) );
		}
		...
		static function add_actions()
		{
			...
			add_action( 'edit_post_form', array( __CLASS__, 'edit_post_form' ) );

			// Delete file via Ajax
			add_action( 'wp_ajax_senddeleterequest', array( __CLASS__, 'wp_ajax_deleterequestedfile' ) );

			// allow reordering
			add_action( 'wp_ajax_rwmb_rearrangefiles', array( __CLASS__, 'wp_ajax_rearrangefiles' ) );
		}
		 ...
		 * @return void
		 */
		static function wp_ajax_rearrangefiles()
		{
		 ...
		 * @return void
		 */
		static function edit_post_form()
		{
		 ...
		 * @return void
		 */
		static function wp_ajax_deleterequestedfile()
		{
		...
		$current_user = wp_get_current_user(); 
			...
			self::delete_image_gallery( $attachment_id );
			...
			$wpdb->delete($service_finder_Tables->attachments,$where);

			if ( $ok )
				wp_send_json_success();
			else
				wp_send_json_error( esc_html__( 'Error: Cannot delete file', 'service-finder' ) );
		}
		
		static function delete_image_gallery($aid){
			global $wpdb, $service_finder_Tables;
			
			$sql = $wpdb->prepare("DELETE FROM ".$service_finder_Tables->attachments." WHERE attachmentid = %d",$aid);
		...
		{
			$i18n_title = apply_filters( 'rwmb_file_upload_string', _x( 'Upload Files', 'file upload', 'service-finder' ), $field );
			$i18n_more  = apply_filters( 'rwmb_file_add_string', _x( '+ Add new file', 'file upload', 'service-finder' ), $field );
		...
		{
			$i18n_delete = apply_filters( 'rwmb_file_delete_string', _x( 'Delete', 'file upload', 'service-finder' ) );
			$i18n_edit   = apply_filters( 'rwmb_file_edit_string', _x( 'Edit', 'file upload', 'service-finder' ) );
			$li          = '
			<li id="item_%s">
			...
			return sprintf(
				...
				$mime_type,
				esc_attr($i18n_edit),
				esc_url(get_edit_post_link( $attachment_id )),
				$i18n_edit,
				esc_attr($i18n_delete),
				esc_attr($attachment_id),
				$i18n_delete
			);
		}
		...
		static function value( $new, $old, $post_id, $field )
		{
			...
			$new   = array();
			$files = self::staticfile_array( $_FILES[$name] );
		 ...
		 * @return array
		 */
		static function staticfile_array( $files )
		{
			$output = array();
