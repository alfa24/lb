...
';
			...
			$nestedData[] = '
<button type="button" data-id="'.esc_attr(strtotime($result->date)).'" class="btn btn-primary btn-xs editUnAvilabilityButton"><i class="fa fa-pencil"></i>'.esc_html__('Edit','service-finder').'</button>
';
	...
	public function service_finder_getUnAailabilityStartTime($arg){
		...
		$sql = 'SELECT * FROM '.$service_finder_Tables->unavailability.' WHERE date >= CURDATE() AND availability_method = "starttime" AND `provider_id` = '.$user_id;
		if( !empty($requestData['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
			$sql.=" AND ( day LIKE '".$requestData['search']['value']."%' ";    
			$sql.=" OR single_start_time LIKE '".$requestData['search']['value']."%' )";
		}
		$sql.= ' GROUP BY date';
...
';
	...
	public function service_finder_loadUnAailability($arg){
			...
			$data = $wpdb->get_row($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->unavailability.' WHERE `date` = "%s" AND availability_method = "timeslots" AND `provider_id` = %d GROUP BY date',$date,$user_id));
			if(!empty($data)){
			
			$dayname = date('l', strtotime( $date));
			$results = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->timeslots.' AS timeslots WHERE (SELECT COUNT(*) FROM '.$service_finder_Tables->bookings.' AS bookings WHERE `bookings`.`status` != "Cancel" AND `bookings`.`date` = "%s" AND `bookings`.`start_time` = `timeslots`.`start_time` AND `bookings`.`end_time` = `timeslots`.`end_time`) < `timeslots`.`max_bookings` AND `timeslots`.`provider_id` = %d AND `timeslots`.`day` = "%s"',$date,$user_id,strtolower($dayname)));
			
			$qry = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->unavailability.' WHERE `date` = "%s" AND availability_method = "timeslots" AND provider_id = %d',$date,$user_id));
...
';
			...
			$res2 = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->unavailability.' WHERE `provider_id` = %d AND availability_method = "timeslots" AND wholeday = "yes" GROUP BY date',$user_id));
			
			$bookings = $wpdb->get_results($wpdb->prepare('SELECT date, COUNT(ID) as totalbooked FROM '.$service_finder_Tables->bookings.' WHERE `provider_id` = %d AND date > now() GROUP BY date',$user_id));
			if(!empty($bookings)){
			...
			$getalloteddates = $wpdb->get_results($wpdb->prepare('SELECT date FROM '.$service_finder_Tables->unavailability.' WHERE `provider_id` = %d AND availability_method = "timeslots" AND wholeday != "yes" AND date > now() GROUP BY date',$user_id));
	...
	public function service_finder_loadUnAailabilityStartTime($arg){
			...
			$data = $wpdb->get_row($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->unavailability.' WHERE `date` = "%s" AND availability_method = "starttime" AND `provider_id` = %d GROUP BY date',$date,$user_id));
			if(!empty($data)){
			
			$dayname = date('l', strtotime( $date));
			$results = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->starttime.' AS starttime WHERE (SELECT COUNT(*) FROM '.$service_finder_Tables->bookings.' AS bookings WHERE `bookings`.`status` != "Cancel" AND `bookings`.`date` = "%s" AND `bookings`.`start_time` = `starttime`.`start_time`) < `starttime`.`max_bookings` AND `starttime`.`provider_id` = %d AND `starttime`.`day` = "%s"',$date,$user_id,strtolower($dayname)));
			
			$qry = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->unavailability.' WHERE `date` = "%s" AND availability_method = "starttime" AND provider_id = %d',$date,$user_id));
			...
			$res2 = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->unavailability.' WHERE `provider_id` = %d AND availability_method = "starttime" AND wholeday = "yes" GROUP BY date',$user_id));
			...
			$getalloteddates = $wpdb->get_results($wpdb->prepare('SELECT date FROM '.$service_finder_Tables->unavailability.' WHERE `provider_id` = %d AND availability_method = "starttime" AND wholeday != "yes" AND date > now() GROUP BY date',$user_id));
	...
	public function service_finder_resetCalender($attrs = ''){
			...
			$res = $wpdb->get_results($wpdb->prepare('SELECT * FROM '.$service_finder_Tables->unavailability.' WHERE `provider_id` = %d AND wholeday = "yes" GROUP BY date',$user_id));
			...
			$getalloteddates = $wpdb->get_results($wpdb->prepare('SELECT date FROM '.$service_finder_Tables->unavailability.' WHERE `provider_id` = %d AND wholeday != "yes" AND date > now() GROUP BY date',$user_id));
