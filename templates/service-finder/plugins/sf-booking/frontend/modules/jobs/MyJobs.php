		...
		foreach($jobs as $job){
			...
			if(get_post_meta($job,'_filled',true)){
				...
				if($bookinginfo->status == 'Cancel'){
					...
					$bookingstatus = ucfirst($bookinginfo->status);
				}
				}else{
			...
			$viewapplyjob = '<button type="button" class="btn btn-custom btn-xs viewapplyjob" data-jobid="'.esc_attr($job).'" title="'.esc_html__('View Applied Job Quotation', 'service-finder').'"><i class="fa fa-eye"></i></button>';
			...
			$nestedData[] = $viewapplyjob;
			}else{
			$nestedData[] = $approvallink.' '.$statusbtn.' '.$viewapplyjob;
			}
