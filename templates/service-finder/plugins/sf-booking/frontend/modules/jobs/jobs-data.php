...
// show messages
$redirect = add_query_arg( array('manageaccountby' => 'admin','manageproviderid' => $provider_id,'joblimitplanupdate' => 'success'), service_finder_get_url_by_shortcode('[service_finder_my_account') );
	...
	if(!empty($tokenRow)){
		...
		if( is_array($checkoutDetails) && ($checkoutDetails['ACK'] == 'Success') ) {
				...
				if( is_array($singlePayment) && $singlePayment['ACK'] == 'Success') {
					...
					$wpdb->insert($service_finder_Tables->transaction,wp_unslash($txndata));
					// show messages
					$redirect = add_query_arg( array('joblimitplanupdate' => 'success'), service_finder_get_url_by_shortcode('[service_finder_my_account') );
					wp_redirect($redirect);
					die;
...
if(isset($_GET['joblimitpayment_made']) && ($_GET['joblimitpayment_made'] == 'cancel') && !empty($_GET['token'])){
	...
	if($tokenRow){
		...
		$errors = new WP_Error();
		$message = esc_html__("You canceled payment. Your payment wasn't maid","service-finder");
...
if(isset($_GET['joblimitpaymentmade']) && $_GET['joblimitpaymentmade'] == 'success' && $_GET['payutransaction'] == 'success' && isset($_GET['payutransaction']) && isset($_POST['payuMoneyId']) && isset($_POST['status'])){
...
global $wpdb, $stripe_options, $service_finder_options, $service_finder_Tables;
		...
		try {			
			...
			if ($charge->paid == true && $charge->status == "succeeded") { 
				...
				$msg = (!empty($service_finder_options['subscription-cancel'])) ? $service_finder_options['subscription-cancel'] : esc_html__('Payment has been made successfully', 'service-finder');
				$success = array(
						'status' => 'success',
						'suc_message' => $msg
						);
...
$cost = get_user_meta($current_user->ID,'job_applications_cost',esc_html($jobid).'-'.esc_html($costing));
$description = get_user_meta($current_user->ID,'job_applications_description',esc_html($jobid).'-'.esc_html($costing));
