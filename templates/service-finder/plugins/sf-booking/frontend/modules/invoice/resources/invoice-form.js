...
// When the browser is ready...
  jQuery(function() {
	...
	jQuery('#invoice-Modal').on('hide.bs.modal', function() {
		...
		jQuery('.add-invoice #total_amount').html('0.00');
		jQuery('.add-invoice #total_discount').html('0.00');
		jQuery('.add-invoice #total_tax').html('0.00');
		jQuery('.add-invoice #grand_total').html('0.00');
	  ...
	  jQuery.ajax({
						...
						success:function (data, textStatus) {
							...
							jQuery('#editInvoice')
								...
								.find('#total_amount').html(data['total']).end()
								.find('#total_discount').html(data['discount']).end()
								.find('#total_tax').html(data['tax']).end()
								.find('#grand_total').html(data['grand_total']).end();
	 ...
	 jQuery('body').on('click', '.sendReminder', function(){
			...
			bootbox.dialog({
                ...
                message: '<div class="row">  ' +
                    ...
                    '<div class=""> ' +
                    '<input name="submit" type="submit" class="btn btn-success" value="Send Reminder"> ' +
                    '</div> ' +
	...
	jQuery('.edit-invoice')
	  ...
	  min: 1,
		...
		function service_finder_calculateTotal(formid){
			...
			jQuery('#'+formid).find('#total_discount').html(discount.toFixed(2));
			jQuery('#'+formid).find('#total_tax').html(tax.toFixed(2));
			jQuery('#'+formid).find('#total_amount').html(total.toFixed(2));
			jQuery('#'+formid).find('#grand_total').html(grand_total.toFixed(2));
		}
