...
class SERVICE_FINDER_Invoice{
	...
	public function service_finder_getInvoice($arg){
		...
		$invoices = $wpdb->get_results($wpdb->prepare('SELECT invoice.customer_email, invoice.adminfee, invoice.id, invoice.reference_no, invoice.duedate, invoice.grand_total, invoice.status, invoice.txnid FROM '.$service_finder_Tables->invoice.' as invoice WHERE `invoice`.`provider_id` = %d',$user_id));
		...
		$sql = $wpdb->prepare('SELECT invoice.customer_email, invoice.adminfee, invoice.id, invoice.reference_no, invoice.duedate, invoice.grand_total, invoice.status, invoice.txnid FROM '.$service_finder_Tables->invoice.' as invoice WHERE `invoice`.`provider_id` = %d',$user_id);
		$query=$wpdb->get_results($sql);
		$totalData = count($query);
		$totalFiltered = $totalData;  // when there is no search parameter then total number rows = total number filtered rows.
		
		
		$sql = "SELECT invoice.customer_email, invoice.adminfee, invoice.id, invoice.reference_no, invoice.duedate, invoice.grand_total, invoice.status, invoice.txnid";
...
<div class="checkbox">
  ...
  <label for="invoice-'.esc_attr($result->id).'"></label>
			...
			if($result->status == 'pending' && strtotime($date) < $now){
				...
				$status = $status = service_finder_translate_static_status_string($result->status);
			}
...
$flag = 1;
		...
		if($flag == 1){
		$paybtn = '
<input name="invoicepayment" id="invoicepayment" type="submit" value="Pay Now" class="btn btn-primary center-block">
';
		}else{
		$paybtn = '
<div>'.esc_html__('No Payment Method Available', 'service-finder').'</div>
';
		}	
		$payform = '';
		if($row->status != 'paid'){
		$payform = '
<form class="myform pay-now" method="post">
  <div class="col-md-12">
...
'.$payform;
	...
	if($pay_booking_amount_to == 'admin'){
		...
		$twocheckoutaccountid = esc_html($settings['twocheckoutaccountid']);
			...
			$nestedData[] = '<div class="booking-price">'.$result->duedate.'</div>';
			$nestedData[] = '<div class="booking-price">'.$result->grand_total.'</div>';
...
';
			...
			$nestedData[] = '
<button type="button" class="btn btn-primary btn-xs viewInvoice" data-twocheckoutpublishkey="'.esc_attr($twocheckoutpublishkey).'" data-twocheckoutmode="'.esc_attr($twocheckoutmode).'" data-twocheckoutaccountid="'.esc_attr($twocheckoutaccountid).'" data-id="'.esc_attr($result->id).'"><i class="fa fa-eye"></i>View</button>
';
	...
	public function service_finder_editInvoiceData($arg){
		...
		if(!empty($services)){
			...
			$dropdownitem = service_finder_getServices($user_id);
...
<div class="form-group clearfix" >
  <div class="col-xs-3">
    <select title="Services" name="service_title['.$key.']" data-index="'.esc_attr($key).'" class="form-control">
  ...
  <div class="col-xs-3">
    <input type="text" name="service_desc['.$key.']" value="'.esc_attr($service[3]).'" data-index="'.esc_attr($key).'" class="form-control" placeholder="'.esc_html__('Description', 'service-finder').'">
  </div>
  <div class="col-xs-2">
    <input type="text" name="service_price['.$key.']" value="'.esc_attr($service[4]).'" data-index="'.esc_attr($key).'" class="form-control text-right" placeholder="'.esc_html__('Price', 'service-finder').'">
  </div>
  <div class="col-xs-1">
...
<div class="form-group hide clearfix " id="serviceEditTemplate">
  <div class="col-xs-3">
    <select title="Services" name="service_title" data-index="" class="form-control">
      <option value="new">'.esc_html__('New Service', 'service-finder').'</option>
  ...
  <div class="col-xs-3">
    <input type="text" name="service_desc" data-index="" class="form-control" placeholder="'.esc_html__('Description', 'service-finder').'">
  </div>
  <div class="col-xs-2">
    <input type="text" name="service_price" data-index="" class="form-control text-right" placeholder="'.esc_html__('Price', 'service-finder').'">
  </div>
  <div class="col-xs-1">
    <button type="button" class="btn btn-default removeButton"><i class="fa fa-minus"></i></button>
  </div>
			...
			if ( ! $invoice_id ) {
				...
				if(function_exists('service_finder_add_notices')) {
					$noticedata = array(
							...
							'notice' => esc_html__('New Invoice generated. Please pay it soon.', 'service-finder')
							);
...
<a href="'.esc_url($userLink).'">'.esc_html__('Pay Now','service-finder').'</a>';
			...
			$tokens = array('%REFERENCENO%','%DUEDATE%','%PROVIDERNAME%','%DISCOUNTTYPE%','%DISCOUNT%','%TAXTYPE%','%TAX%','%DISCRIPTION%','%TOTAL%','%GRANDTOTAL%');
			
			$replacements = array($maildata['reference_no'],$maildata['duedate'],service_finder_get_providername_with_link($row->wp_user_id),$discounttype,$discount,$taxtype,$tax,$description,$maildata['total'],$maildata['grand_total']);
			...
			if ( ! $res) {
				...
				if(function_exists('service_finder_add_notices')) {
					...
					$noticedata = array(
							...
							'topic' => esc_html__('Invoice Update', 'service-finder'),
							'notice' => esc_html__('Previousaly generated invoice have been updated. Please review it.', 'service-finder')
							);
					service_finder_add_notices($noticedata);
