...
if($getcity != "" && $getcountry != ""){
	$address = str_replace(" ","+",$getcity).'+'.str_replace(" ","+",$getcountry);
	...
	$address = str_replace(" ","+",$getcountry);
	$res = service_finder_getLatLong($address);
	$lat = $res['lat'];
	$lng = $res['lng'];
...
wp_add_inline_script( 'bootstrap', 'jQuery(document).ready(function($) {
				...
				jQuery("body").on("click", "#viewTypes li", function(){
					...
					window.location = homeurl+"?s=&searchAddress='.esc_js($srhaddress).'&keyword='.esc_js($keyword).'&catid='.esc_js($getcatid).'&country='.esc_js($getcountry).'&city='.esc_js($getcity).'&pagenum="+page+"&viewtype="+viewtype+"&numberofpages="+numberofpages+"&setorderby="+setorderby+"&setorder="+setorder;
				...
				jQuery("body").on("change", "#numberofpages", function(){
						...
						var homeurl = "'.esc_js(home_url("/")).'";
						window.location = homeurl+"?s=&searchAddress='.esc_js($srhaddress).'&keyword='.esc_js($keyword).'&catid='.esc_js($getcatid).'&country='.esc_js($getcountry).'&city='.esc_js($getcity).'&pagenum=1&viewtype="+viewtype+"&numberofpages="+numberofpages+"&setorderby="+setorderby+"&setorder="+setorder;
				});
				//Set Order by
				jQuery("body").on("change", "#setorderby", function(){
						var page = jQuery(".pagination span.current").html();
...
$setorder = (isset($_GET['setorder'])) ? $_GET['setorder'] : '';
   ...
   $getProviders = new SERVICE_FINDER_searchProviders();
	...
	$address = (isset($_GET['address'])) ? esc_html($_GET['address']) : '';
   ...
   $providersInfoArr = $getProviders->service_finder_getSearchedProviders(esc_attr($distance),esc_attr($minprice),esc_attr($maxprice),esc_attr($keyword),esc_attr($address),esc_attr($city),esc_attr($catid),esc_attr($country),$start,$per_page,$orderby,$order);
...
wp_add_inline_script( 'bootstrap', 'jQuery(document).ready(function($) {
				...
				var pageurl = baseurl+"?s=&keyword='.esc_js($keyword).'&distance='.esc_js($distance).'&minprice='.esc_js($minprice).'&maxprice='.esc_js($maxprice).'&catid='.esc_js($getcatid).'&country='.esc_js($getcountry).'&city='.esc_js($getcity).'&pagenum="+startpage+"&viewtype="+viewtype+"&numberofpages="+numberofpages+"&setorderby="+setorderby+"&setorder="+setorder;
				...
				jQuery("body").on("click", ".cvf_universal_container .cvf-universal-pagination li.activelink a", function(){
					...
					var pageurl = baseurl+"?s=&keyword='.esc_js($keyword).'&distance='.esc_js($distance).'&minprice='.esc_js($minprice).'&maxprice='.esc_js($maxprice).'&catid='.esc_js($getcatid).'&country='.esc_js($getcountry).'&city='.esc_js($getcity).'&pagenum="+page+"&viewtype="+viewtype+"&numberofpages="+numberofpages+"&setorderby="+setorderby+"&setorder="+setorder;
					 if(pageurl!=window.location){
						window.history.pushState({path:pageurl},"",pageurl);
					}
