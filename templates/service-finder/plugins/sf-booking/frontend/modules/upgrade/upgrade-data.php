...
if(isset($_GET['featurepayment_made']) && ($_GET['featurepayment_made'] == 'cancel') && !empty($_GET['token'])){
	...
	if($tokenRow){
		...
		$errors = new WP_Error();
		$message = esc_html__("You canceled payment. Your payment wasn't maid","aone");
...
if(isset($_GET['featurepayment']) && $_GET['featurepayment'] == 'success' && $_GET['payutransaction'] == 'success' && isset($_GET['payutransaction']) && isset($_POST['payuMoneyId']) && isset($_POST['status'])){
...
$where = array(
		'provider_id' => esc_attr($_POST['providerid']),
		);
