...
// When the browser is ready...
  jQuery(function() {
	...
	jQuery('.upgrade-form')
		...
		.on('change', 'input[name="provider-role"]', function() {
		if(jQuery(this).hasClass("current") || jQuery(this).hasClass("free")){
	...
	jQuery('.feature-form')
        ...
        .on('success.form.bv', function(form) {
				...
				if(payment_mode == 'paypal' || payment_mode == 'payumoney'){
					return true;
		  ...
		  if(result){
			...
			jQuery.ajax({
						...
						success:function (data, textStatus) {
							jQuery('.loading-area').hide();
							if(data['status'] == 'success'){
								window.location.href= currenturl+'?cancelfeatured=success';	
							}else{
								jQuery( "<div class='alert alert-danger'>"+data['err_message']+"</div>" ).insertBefore( "form.upgrade-form" );
							}
