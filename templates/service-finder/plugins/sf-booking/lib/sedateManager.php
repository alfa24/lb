...
abstract class SERVICE_FINDER_sedateManager
{
	...
	protected function service_finder_getProvidersList(){
		...
		$providers = $wpdb->get_results('SELECT * FROM '.$service_finder_Tables->providers.' WHERE full_name LIKE "a%" AND identity = "approved" admin_moderation = "approved" AND account_blocked != "yes"');
		}else{
	...
	protected function service_finder_getAllProvidersList(){
		...
		$restrictuserarea = (isset($service_finder_options['restrict-user-area'])) ? esc_attr($service_finder_options['restrict-user-area']) : '';
		if($restrictuserarea && $identitycheck){
		$providers = $wpdb->get_results('SELECT * FROM '.$service_finder_Tables->providers.' WHERE admin_moderation = "approved" AND identity = "approved" account_blocked != "yes" ORDER BY full_name');
		}else{
