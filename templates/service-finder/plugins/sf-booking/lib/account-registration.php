...
if((isset($_POST['payment_mode']) || isset($_POST['pay_mode']) || (isset($_POST['freemode']) && $freemode == 'yes')) && ($payment_mode == 'paypal' || $payment_mode == 'wired' || (isset($_POST['freemode']) && $freemode == 'yes') || ($pay_mode == 'paypal_upgrade' || $pay_mode == 'skippayment')) && isset($_POST['user-register'])) {

	// register user with minimal role
	$upgrade = false;
	$pay_mode = (isset($_POST['pay_mode'])) ? $_POST['pay_mode'] : '';
	if($pay_mode == 'paypal_upgrade' || $pay_mode == 'skippayment' || ($freemode == 'yes' && $_POST['userregister'] == 'upgrade')){
		$upgrade = true;
	...
	if(is_wp_error( $userId )){
		...
		if(isset($get_provider_role) || $freemode == 'yes'){
			...
			if (($role == "package_1") || ($role == "package_2") || ($role == "package_3") || ($freemode == 'yes')){
				...
				$type = '';
				// non free
				if( isset($service_finder_options['enable-paypal']) && (!$free) && $pay_mode != 'skippayment' && $payment_mode != 'wired' ){
					...
					if($upgrade){
						...
						if(service_finder_get_url_by_shortcode('[service_finder_success_message]') == '' && $pay_mode == 'skippayment'){
							$redirect = add_query_arg( array('manageaccountby' => 'admin','manageproviderid' => $userId,'upgrade' => 'success'), service_finder_get_url_by_shortcode('[service_finder_my_account]') );
						}else{
							$redirect = add_query_arg( array('upgrade' => 'success'), service_finder_get_url_by_shortcode('[service_finder_my_account]') );
						}
					} else {
						...
						$registerMessages = (!empty($service_finder_options['provider-signup-successfull'])) ? $service_finder_options['provider-signup-successfull'] : esc_html__('Your provider account was activated! Check your email address for password', 'service-finder');
		...
		if( is_array($checkoutDetails) && ($checkoutDetails['ACK'] == 'Success') ) {
			...
			if (isset($checkoutDetails['BILLINGAGREEMENTACCEPTEDSTATUS']) && $checkoutDetails['BILLINGAGREEMENTACCEPTEDSTATUS']) {
				...
				if (!empty($oldProfile)) {
					...
					if($res['ACK'] != 'Success'){
						$redirect = add_query_arg( array('upgrade' => 'failed'), service_finder_get_url_by_shortcode('[service_finder_my_account]') );
				...
				if( is_array($recurringPayment) && $recurringPayment['ACK'] == 'Success' ) {
					...
					if(isset($_GET['upgrade'])){
						...
						$registerMessages = (!empty($service_finder_options['provider-recurring-signup-successfull'])) ? $service_finder_options['provider-recurring-signup-successfull'] : esc_html__('PayPal recurring payments profile created. Your provider account was activated! Check your email address for password', 'service-finder');
						$pageid = (!empty($service_finder_options['signup-redirect-option'])) ? $service_finder_options['signup-redirect-option'] : '';
						if($pageid == 'no'){
						$redirect = add_query_arg( array('created' => 'success'), service_finder_get_url_by_shortcode('[service_finder_success_message]') );
						}else{
...
if((isset($_POST['payment_mode']) || isset($_POST['pay_mode'])) && ($payment_mode == 'payumoney' || $pay_mode == 'payumoney_upgrade') && isset($_POST['user-register'])) {
	...
	if(isset($get_provider_role)){
		...
		if (($role == "package_1") || ($role == "package_2") || ($role == "package_3") || ($freemode == 'yes')){
			...
			$type = '';
				...
				$surl = ($upgrade) ? add_query_arg( array('registerviapayumoney' => 'success','upgrade' => '1','payutransactionforreg' => 'success'), home_url('/my-account/') ) : add_query_arg( array('registerviapayumoney' => 'success','payutransactionforreg' => 'success'), home_url('/my-account/') );
				
				$furl = ($upgrade) ? add_query_arg( array('registerviapayumoney' => 'failed','payutransactionforreg' => 'failed','upgrade' => '1'), home_url('/my-account/') ) : add_query_arg( array('registerviapayumoney' => 'failed','payutransactionforreg' => 'failed'), home_url('/my-account/') );
...
global $service_finder_options;
	...
	if( isset($service_finder_options['stripe-type']) && $service_finder_options['stripe-type'] == 'test' ){
		...
		$secret_key = (!empty($service_finder_options['stripe-test-public-key'])) ? $service_finder_options['stripe-live-secret-key'] : '';
		$public_key = (!empty($service_finder_options['stripe-test-public-key'])) ? $service_finder_options['stripe-live-public-key'] : '';
	}
	Stripe::setApiKey($secret_key);
	try {

	$currentcustomer = Stripe_Customer::retrieve($customerID);
...
$captchaon = isset($arg['captchaon']) ? $arg['captchaon'] : 0;
if($captchaon == 1){

	if($arg['signup_user_role'] == 'Provider'){
		
		if((empty($_SESSION['captcha_code_providersignup'] ) || strcasecmp($_SESSION['captcha_code_providersignup'], $arg['captcha_code']) != 0) && (strcasecmp($_SESSION['captcha_code_providersignuppage'], $arg['captcha_code']) != 0 || empty($_SESSION['captcha_code_providersignuppage'] ))){  
			$error = array(
					'status' => 'error',
		...
		if((empty($_SESSION['captcha_code_customersignup'] ) || strcasecmp($_SESSION['captcha_code_customersignup'], $arg['captcha_code']) != 0) && (strcasecmp($_SESSION['captcha_code_customersignuppage'], $arg['captcha_code']) != 0 || empty($_SESSION['captcha_code_customersignuppage'] ))){  
			$error = array(
					'status' => 'error',
					'err_message' => esc_html__('The Validation code does not match!', 'service-finder'),
					);
...
if(email_exists( esc_attr($arg['signup_user_email']) )){
	...
	$comment_post = array(
        'post_title' => $sanitized_user_name,
        'post_status' => 'publish',
	...
	if($arg['signup_user_role'] == 'Provider'){
		...
		$data = array(
					...
					'company_name' => esc_attr($arg['signup_company_name']),
					
					'full_name' => esc_attr($arg['signup_first_name']).' '.esc_attr($arg['signup_last_name']),

					'email' => esc_attr($arg['signup_user_email']),
					
					'phone' => esc_attr($arg['signup_phone']),

					'category_id' => esc_attr($arg['signup_category']),

					'address' => esc_attr($arg['signup_address']),

					'apt' => esc_attr($arg['signup_apt']),

					'city' => esc_attr($arg['signup_city']),

					'state' => esc_attr($arg['signup_state']),

					'zipcode' => esc_attr($arg['signup_zipcode']),

					'country' => esc_attr($arg['signup_country']),
...
$creds = array();
	...
	$creds['user_login'] = esc_attr($user_name);

	$creds['user_password'] = esc_attr($password);
...
$success = '';
        ...
        if(  email_exists(trim($user_login)) || username_exists( trim($user_login) )) {
            ...
            if( $update_user ) {
				...
				$message = sprintf( esc_html__( 'Your new password is: %s', 'service-finder' ), $random_password);
			
				$msg_body = $message;
				$msg_subject = esc_html__( 'Your new password', 'service-finder' );
				...
				$service_finder_Errors->add( 'update_password', esc_html__( 'ERROR: Oops something went wrong updaing your account.' , 'service-finder') );
				return $service_finder_Errors;
            }
...
global $wpdb, $service_finder_Tables, $service_finder_options;			
			...
			$replacements = array($args['username'],$args['email'],$args['address'],$args['city'],$args['country'],$args['zipcode'],$args['category'],$args['package_name'],$args['payment_type']);
			$msg_body = str_replace($tokens,$replacements,$message);
